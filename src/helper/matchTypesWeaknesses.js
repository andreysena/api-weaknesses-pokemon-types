...
export default async (type1, type2 = false) => {
    ...
    function findFinalWeaknesses(forces, weaknesses, half_damage_from, immunities){
        ...
        weaknesses = weaknesses.reduce((selectWeaknesses, weakness) => {
            return forces.includes(weakness.name) ? selectWeaknesses : [...selectWeaknesses, weakness]
        }, []);

        weaknesses = weaknesses.reduce((selectWeaknesses, weakness) => {
            return immunities.includes(weakness.name) ? selectWeaknesses : [...selectWeaknesses, weakness]
        }, []);

        weaknesses = weaknesses.reduce((selectWeaknesses, weakness) => {
            return half_damage_from.includes(weakness.name) ? selectWeaknesses : [...selectWeaknesses, weakness]
        }, []);
